...
class AW_Blog_Block_Abstract extends Mage_Core_Block_Template {
    ...
    public function addTopLink() {
        if (Mage::helper('blog')->getEnabled()) {
			...
			if ( (is_object($this->getParentBlock())) and (method_exists($this->getParentBlock(), 'addLink')) ) $this->getParentBlock()->addLink($title, $route, $title, true, array(), 15, null, 'class="top-link-blog"');
        }
    }
