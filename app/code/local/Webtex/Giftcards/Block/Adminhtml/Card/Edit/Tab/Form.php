    ...
    protected function _prepareForm()
    {
        ...
        $cardFieldset->addField('card_status', 'select', array(
            ...
            'options' => array(
                ...
                '2' => Mage::helper('giftcards')->__('Pending'),
            ),
        ));
        ...
        $offlineFieldset = $form->addFieldset('offline_form', array(
            'legend' => Mage::helper('giftcards')->__('Offline Address (for offline cards only)')
        ));

        $offlineFieldset->addField('offline_country', 'text', array(
            'name' => 'offline_country',
            'label' => Mage::helper('giftcards')->__('Country'),
        ));

        $offlineFieldset->addField('offline_state', 'text', array(
            'name' => 'offline_state',
            'label' => Mage::helper('giftcards')->__('State/Province'),
        ));

        $offlineFieldset->addField('offline_city', 'text', array(
            'name' => 'offline_city',
            'label' => Mage::helper('giftcards')->__('City'),
        ));

        $offlineFieldset->addField('offline_street', 'text', array(
            'name' => 'offline_street',
            'label' => Mage::helper('giftcards')->__('Street'),
        ));

        $offlineFieldset->addField('offline_zip', 'text', array(
            'name' => 'offline_zip',
            'label' => Mage::helper('giftcards')->__('Zip/Postcode'),
        ));

        $offlineFieldset->addField('offline_phone', 'text', array(
            'name' => 'offline_phone',
            'label' => Mage::helper('giftcards')->__('Phone'),
        ));
