...
class Appmerce_Eway_Model_Process extends Varien_Object
{
    /**
     ...
     *
     * @return Appmerce_Eway_Model_Api
     */
    protected function getApi()
    {
        return Mage::getSingleton('eway/api');
    }
    ...
    public function invoice($order)
    {
        ...
        if (!$order->hasInvoices() && $order->canInvoice()) {
            ...
            if ($invoice->getTotalQty() > 0) {
                ...
                $transaction->save();
                $invoice->addComment(Mage::helper('eway')->__('Automatic invoice.'), false);
    ...
    /**
     * Payment review process [multi-method]
     *
     * Update orders with explicit 'payment_review' status. Restore quote.
     *
     * @param $order object Mage_Sales_Model_Order
     * @param $note string Backend order history note
     * @param $transactionId string Transaction ID
     * @param $redirect boolean
     */
    public function review(Mage_Sales_Model_Order $order, $note, $transactionId, $responseCode = 1, $frontend = false)
    {
        $this->check($order);
        if ($order->getId() && $responseCode != $order->getPayment()->getAppmerceResponseCode()) {
            # don't set response code, allow for another status update
            ...
            // Set pending_payment state
            $reviewOrderStatus = $this->getApi()->getConfig()->getReviewStatus($paymentMethodCode);
            $order->setState(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW, $fraudOrderStatus, $note, $notified = false);
            $order->save();
        }
    ...
    public function cancel(Mage_Sales_Model_Order $order, $note, $transactionId, $responseCode = 1, $frontend = false)
    {
        ...
        if ($frontend) {
            $this->restore();
            $message = Mage::helper('eway')->__('Payment failed. Please try again.');
    ...
    {
        $this->getCheckout()->setQuoteId($this->getCheckout()->getEwayQuoteId(true));
        $this->getCheckout()->setLastOrderId($this->getCheckout()->getEwayOrderId(true));
    }
