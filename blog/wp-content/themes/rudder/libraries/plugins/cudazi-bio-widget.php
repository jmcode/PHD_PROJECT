...
class Cudazi_BioWidget extends WP_Widget {
    ...
    function widget($args, $instance) {		
		...
		$disable_search = $instance['disable_search'];
		...
		if ( $social_icon_output ) {
			$social_icon_output = "<ul class='social-networks'>" . $social_icon_output . "</ul>";						
		}
		...
		<?php } ?>		
		<?php if ( ! $disable_search ) { get_search_form(); } ?>
    ...
    function update($new_instance, $old_instance) {				
		...
		$instance['disable_search'] = $new_instance['disable_search'];
		return $instance;
    }
    ...
    function form($instance) {		
    	...
    	if ( ! isset(  $instance['disable_search'] ) ) {
    		$instance['disable_search'] = '';
    	}
        
		$photo_url = esc_attr($instance['photo_url']);
		$photo_linkto = esc_attr($instance['photo_linkto']);
		$description = esc_html($instance['description']);
		$disable_search = esc_html($instance['disable_search']);
        ...
        ?>
            ...
            <p>
            	<label for="<?php echo $this->get_field_id('disable_search'); ?>">
            		<input type="checkbox" id="<?php echo $this->get_field_id('disable_search'); ?>" name="<?php echo $this->get_field_name('disable_search'); ?>" <?php if ( $disable_search ) { echo "checked='checked'"; } ?> /> <?php _e('Disable Search Form', 'cudazi'); ?> 
            	</label>            	
            </p>
